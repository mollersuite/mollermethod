...
import {
	...
	SpringOptions,
	useMotor,
	useSpring as useNumberSpring,
} from "hooks/common/flipper-hooks"
...
const supportedTypes = {
	...
	Color3: (color: Color3, options: SpringOptions): Binding<Color3> => {
		...
		motor.setGoal([
			new Spring(color.R, options),
			new Spring(color.G, options),
			new Spring(color.B, options),
		])
	...
	UDim2: (udim2: UDim2, options: SpringOptions): Binding<UDim2> => {
		...
		return getBinding(motor).map(
			([xS, xO, yS, yO]) => new UDim2(xS, math.round(xO), yS, math.round(yO))
		)
	},
